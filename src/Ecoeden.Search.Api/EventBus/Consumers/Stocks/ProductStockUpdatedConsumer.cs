...
public class ProductStockUpdatedConsumer(ILogger logger,
...
) : ConsumerBase<ProductStockUpdated>(eventRecorderService), IConsumer<ProductStockUpdated>
{
    ...
    public async Task Consume(ConsumeContext<ProductStockUpdated> context)
    {
        _logger.Here().MethodEntered();
        _logger.Here()
           .ForContext("MessageId", context.MessageId)
           .ForContext("Event", typeof(ProductStockUpdated).Name)
           .WithCorrelationId(context.Message.CorrelationId)
           .Information("Message processing started for the event {type}", typeof(ProductStockUpdated).Name);

        var summary = await PrepareStockSummary(context.Message);

        if (!summary.IsSuccess) return;
        var result = await _searchService.UpdateDocumentAsync(summary.Data, new() { { "id", summary.Data.Id } }, _elasticSearchOption.StockIndex);
    ...
    private async Task<Result<StockSearchSummary>> PrepareStockSummary(ProductStockUpdated message)
    {
